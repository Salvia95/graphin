...
**이 명령은 버전을 올리지 않는다.** 재설치는 *지금 플러그인 캐시에 있는*
매니페스트를 따르므로, 캐시가 낡았으면 낡은 버전을 다시 설치할 뿐이다. 설치된
버전이 최신 릴리스보다 뒤처져 보이면 그건 이 명령이 아니라 플러그인 업데이트가
필요한 것이다 — `/graphin:doctor` §2가 판정한다.
