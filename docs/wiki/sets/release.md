...
- [§8 변경 통제와 릴리스 게이트](../../rag-bench-spec.md#8-변경-통제와-릴리스-게이트-2026-09-01-소유자-결정) —
  게이트는 버전 자리를 따른다: minor 이상은 풀셋 36태스크 × 3런(`--jobs 3`으로
  ~35분), patch는 스모크 6태스크(~수 분, 5분 내 디스패치 목표), 성능 무관 변경은
  waive(경로 레일이 판단을 검증). 커밋-바인딩 마커를 훅과 release 에이전트 §1이 함께 검사하고,
  측정은 호출자·검증은 에이전트의 단계다. 새 커밋에는 새 측정.
