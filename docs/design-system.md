...
- **카드 오른쪽 버튼은 `Copy`다.** 시안의 `Open set file`·`Retire a term`은 콘솔이
  ...
  `approve`만 진짜 액션(`Review candidate`)을 갖는다. *(2026-08-23에 하나 늘었다 —
  `drift`는 `Repin this`도 갖는다. 카드 문구가 시키는 일이 실제로 API가 됐다.)*

### 0.10 시안을 붙이고 나서야 보인 것 (2026-08-23)

시안의 잘못이 아니라, 목업이 알 수 없던 것들이다. 넷 다 **한 화면에 같은 낱말이
두 뜻으로 있던** 자리다.

- **필터 칩을 단계가 아니라 8종(kind)으로 건다.** 단계가 이미 그룹 헤딩이 됐으므로
  단계 칩은 바로 아래 헤딩과 같은 숫자를 반복했다. 그리고 브리프 §6.1이 걱정한 것은
  4단계가 아니라 **8종을 훑어서 구분하는 것**이었다. 칩은 자기 단계의 글리프와 색을
  입고 kind 이름을 모노로 쓴다 — 카드 왼쪽 거터의 이름과 같은 낱말이다.
- **`Decisions` 타일은 큐만 센다.** 타일이 12, 바로 아래 탭이 6이었다. 백로그는
  한 줄 아래에 남아, 오늘 기다리는 일로 오독될 자리에서 비켰다.
- **모든 그룹이 접힌다.** 백로그만 접혔는데, 파일 하나 옮기면 그 파일을 가리키는
  엔트리가 **전부** dangling이라 alert 그룹도 수십 줄이 될 수 있다.
- **`Adoption` 타일은 문이다.** 퍼센트 하나를 내면서 그 수가 무엇인지 못 보여 주는
  타일은 `usage-spec.md` §4.2.1이 고치려던 고장(분모가 안 보이는 비율)이다. 눌러서
  들어가는 usage 뷰가 **모든 비율을 자기 분자·분모와 함께** 낸다. 근거는
  `console-spec.md` §2의 2026-08-23 항목.

카드 오른쪽 액션 열이 시안보다 넓어졌다. 시안은 카드마다 버튼 하나를 그렸는데,
`glossary_full`과 `unread_set`은 **지시만 있고 누를 게 없던** 자리였다(`Retire a
term` · `Edit summary`, 둘 다 서랍). `Open`은 에디터가 감지됐을 때만 붙고 —
아무 일도 안 하는 링크는 링크가 없는 것보다 나쁘다 — `drift`는 `Repin this`를
갖는다. 열이 비는 카드는 여전히 `Copy` 하나다.

폭 하한은 시안의 `min-width:1280px` 대신 **1120px**이다. 1280은 아트보드 폭이지
요구가 아니고(브리프 §3.4는 "1280 미만에 시간 쓰지 마라"이지 "깨져라"가 아니다),
1280 화면에서 클래식 스크롤바가 있으면 하한이 1280일 때 가로 스크롤이 생긴다.
