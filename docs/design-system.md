...
**웹폰트를 받아오지 않는다.** 콘솔은 사용자 기계에서 도는 로컬 도구이고, 로컬 도구가
외부 호스트에 요청을 거는 것은 오프라인에서 깨지고 온라인에서는 설명할 이유가 없는
연결이다. 폰트 파일을 `go:embed`에 넣지도 않는다 — 바이너리 크기를 그만큼 늘릴 값이
아니다. 그래서 스택은 "설치돼 있으면 쓰고 아니면 시스템 폰트"로 내려간다.
