...
**§13**에 있다. 릴리스 실행 자체는 `.claude/agents/release.md`(서브에이전트)에
절차로 적혀 있고, 그 에이전트는 규칙을 복사해 갖지 않고 이 문서를 그때그때
읽는다 — 규칙이 바뀌면 따라오게 하려는 것이다.
...
### 10.4 배달 실측 (2026-08-08)

**공개는 배달이 아니다.** §10.2가 "업그레이드는 자동으로 돈다"를 보였는데, 그
문장에는 조용한 전제가 하나 있었다: **플러그인이 먼저 갱신되어 있어야 한다.**
그리고 그 갱신은 저절로 일어나지 않는다 — 이 머신에서는 릴리스 셋이 나가는
동안 한 번도 일어나지 않았고, 아무것도 그 사실을 말해 주지 않았다.

| | 2026-08-08 01:20 (측정) |
|---|---|
| 설치된 `graphin@graphin` | **0.2.0** (08-06T14:08 이후 갱신 없음) |
| 설치된 `graphin-guide@graphin` | **0.1.0** (`main`은 0.2.1) |
| 마켓플레이스 캐시 `lastUpdated` | **08-06T15:16** |
| 두 워크스페이스의 `.graphin/binpath` | `graphin-0.2.0-linux-amd64` |

v0.2.1·v0.2.2·v0.2.3이 전부 나갔지만 **한 대도 도착하지 않았다.** 원인은 버그가
아니라 §6.5의 설계다 — 런처는 `$CLAUDE_PLUGIN_ROOT/install/manifest.json`을
기준으로 삼고, `ROOT`는 **설치된 플러그인 캐시 디렉터리**다. 그래서 사슬이
이렇게 된다:

```
마켓플레이스 캐시 갱신 → 플러그인 업데이트 → (재시작) → 런처가 매니페스트 차이를 봄 → 바이너리 교체
```

**앞의 두 칸은 사용자 행동이고, 자동으로 일어나지 않는다.** §13.1-3의 "마켓
플레이스는 `main`을 서빙한다"는 맞지만 그 문장이 답하지 않는 것이 여기 있다 —
서빙은 **업데이트를 실행하는 시점에** 일어난다. `plugin.json`을 올리는 것은
업데이트를 *가능하게* 할 뿐 *발생시키지* 않는다. `graphin-guide`가 릴리스와
무관하게 배달된다는 독법은 그래서 틀렸다.

복구는 두 줄이고, 실측 소요는 21초다:

```
claude plugin marketplace update graphin
claude plugin update graphin@graphin     # 0.2.0 → 0.2.3
claude plugin update graphin-guide@graphin   # 0.1.0 → 0.2.1
```

바이너리는 그다음 세션의 런처가 받는다(예열하려면 `install.sh`를 새 `ROOT`로 직접
실행 — 25MB 다운로드가 MCP 기동 타임아웃 밖에서 끝난다). 정리된 옛 파일 때문에
`binpath`가 잠시 매달리는 것은 §6.7이 이미 다루는 정상 상태다.

**이 사고가 눈에 안 띈 이유는 아무것도 이상해 보이지 않았기 때문이다.**
`/graphin:doctor`는 `installed.json`을 보여줬고 그 값은 자기 자신과 일관됐다 —
0.2.0이 0.2.0으로 잘 설치되어 있었다. 낡았다는 사실은 **바깥과 비교해야만**
보인다. doctor §2가 이제 그 비교를 한다.

측정에 미치는 영향은 별도로 크다. 채택 진단의 권고 ①(`search_hybrid`의
`file`·`line`)도 Phase 1(마크다운 섹션 노드)도 **사용자 세션에서 한 번도 돈 적이
없다.** 반대로 그 덕에 08-07 이후 쌓인 이벤트는 오염이 아니라 0.2.0 베이스라인의
연장이고, **2026-08-08T01:27Z가 재측정의 깨끗한 컷 지점**이 된다
(`docs/eval/2026-08-07-adoption-diagnosis/findings.md`).
...
1. **버전 필드는 의미가 아니라 배달 트리거다.** Claude Code는 `plugin.json`의
   ...
   2026-08-06: 트리가 앞서 나간 상태에서 `plugin update`가 "already at the
   latest version"이라고 답했다). 안 올린 변경은 **아무에게도, 조용히,
   영원히** 가지 않는다.
