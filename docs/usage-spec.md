...
4. **발견 실패**: **심볼형** `search` ≥3이고 graphin 내비 콜 0인 윈도우.
   분모 = **심볼형** `search`가 있는 윈도우.

판정 불가 런(윈도우 종료까지 후속 없음, `g_read` 미종결)은 **inconclusive**로
별도 카운트. 채택률 = 채택 / (채택 + 폴백).

### 4.2.1 검색 형태 — 발견 실패의 분모

원래 정의는 `search`를 전부 셌고, 그래서 **graphin이 영원히 답할 수 없는 일도
실패로 셌다.** 2026-08-07 진단에서 `database is locked`·`^E`·`sqlalchemy.exc`를
찾던 윈도우가 분모에 있었고, 그게 "발견 실패 57%"의 절반을 만들었다
([findings](eval/2026-08-07-adoption-diagnosis/findings.md)). 인덱스가 있다는
가드는 "graphin이 떠 있었다"를 보장할 뿐 **"그 질문이 graphin의 질문이었다"를
보장하지 않는다.**

패턴을 형태로 갈라 심볼형만 센다. 판정 순서(`PatternShape`):

1. **정규화** — 앞뒤 공백, 셸이 남긴 후행 백슬래시, 감싼 따옴표 한 겹을 벗긴다.
2. **정규식** — <code>^$*+?[]{}()|\\</code> 중 하나라도 있으면 정규식·glob이다.
   `.`은 **일부러 뺐다** — 실제 식별자에 가장 흔한 문자라(`sqlalchemy.exc`,
   `foo.go`) 메타로 치면 절반이 정규식이 된다.
3. **토큰 수** — 3토큰 이상은 산문이다. 심볼 검색은 심볼 하나, 많아야 키워드
   하나가 앞에 붙는다(`class PublishBody`). 이 상한이 없으면 첫 단어가 대문자인
   영어 오류 메시지가 전부 심볼이 된다.
4. **식별자 모양** — 토큰 중 하나라도 snake_case·camelCase·PascalCase·
   SCREAMING_CAPS면 심볼이다.

**민짜 소문자 한 단어(`admin`, `publish`)는 심볼이 아니다.** 산문과 구별할 방법이
없기 때문이고, 이 지표의 고장 방향이 *실패의 과대보고*이므로 애매하면 분모에서
빼는 쪽이 맞다. 실측 대가는 크지 않다 — kinder 715 이벤트에서 발견 실패는
17/26(65%) → **9/19(47%)**로 내려가고, 느슨한 규칙(소문자 한 단어 포함)은
12/21(57%)로 옛 수치 쪽으로 되돌아간다. 걷어내진 것은 `^ *$`·`*.go`·
`<title>[^<]*</title>`·`필수 컨텍스트 미충족`이다.

**남은 긴장 하나를 적어 둔다.** Phase 1(마크다운 섹션 노드) 이후로 문서 안
산문 검색은 graphin의 범위 **안**에 들어왔다. 즉 `literal`은 더 이상 영구히
범위 밖인 통이 아니다. 지금은 여전히 심볼형만 세지만 — 발견 실패는 "어디 있나"의
지표이고 문서 내용 검색은 §4.3의 docs 타깃이 따로 본다 — 리포트가 형태별 수를
**항상 함께** 내므로 재측정 때 데이터로 다시 판단할 수 있다.
...
- **검색 형태별 수** (`search_shapes`): symbol / regex / literal / none.
  발견 실패의 분모가 왜 그 크기인지 리포트 안에서 검산할 수 있어야 한다 —
  분모가 안 보이는 비율은 §4.2.1이 고치려던 그 고장이다.
- 클래스 **bigram 전이 행렬**, **일자별 추이**, **main vs 서브에이전트 분리**.
- **code / db / docs 타깃 분리**: 런이 참조한 노드 id로 채택/폴백을 따로 낸다.
  `db.` 접두사는 db, `.md`·`.markdown` 경로(파일 노드와 `#슬러그` 섹션 노드
  모두)는 docs, 나머지는 code다. 단위는 윈도우가 아니라 **런**이다 — "스키마
  질문에 graphin이 채택되는가"는 런의 질문이고, 한 윈도우가 종류가 다른 런을
  여럿 가질 수 있다. 셋은 **분할이 아니라 겹치는 모집단**이다(한 런이 여러
  종류를 다루면 각각에 센다). search가 돌려준 `result_ids`도 분류에 쓴다 —
  스키마 노드를 띄워 놓고 grep으로 도망간 런이야말로 이 분리가 찾으려는
  케이스인데, 탐색된 노드만 보면 그 절반이 통째로 빠진다. 노드 id를 하나도
  참조하지 않은 런은 어느 쪽도 아니므로 `runs`를 함께 낸다.
...
- 진단: **`binpath`는 있는데 `merkle.json`이 없으면** 서버는 떴고 에이전트가
  `bootstrap_workspace`를 한 번도 부르지 않은 것이다. 이것이 채택 실패의 가장
  완전한 형태인데, 인덱스가 없으니 이벤트도 없어 **어떤 지표에도 0%가 아니라
  부재로 집계된다.** 그래서 리포트가 이 조합을 명시적으로 말한다 — 지표가 세지
  못하는 것을 최소한 묻는 사람에게는 숨기지 않는다
  ([2026-08-11 재측정](eval/2026-08-11-adoption-remeasure/findings.md)).
  여러 워크스페이스를 훑는 롤업은 이 명령의 계약 밖이다(로그 하나가 대상이다).

### 5.1 `graphin usage prune`

```
graphin usage prune --before <YYYY-MM-DD|72h> [--log <dir>] [--dry-run] [--json]
```

`--before`에 기본값은 **없다.** 데이터를 지우는 명령이고, 기본값이 있으면
`--log` 오타 한 번이 의도하지 않은 로그를 지운다.

**활성 로그는 제자리에서 걸러내지 않고 먼저 회전시킨다.** `hooks/usage.sh`가
모든 툴콜마다 `usage ingest`를 돌리고 `appendLine`은 락 없이 O_APPEND로
한 줄 쓰고 닫는다. `events.jsonl`을 읽어 거른 사본을 덮어쓰면 그 사이에 쓰인
줄이 사라지는데, 그건 이 패키지에 원래 없는 레이스다. 회전(rename)은 32MiB
로테이션이 이미 하고 있고 그 주석이 남는 레이스 하나("fd를 먼저 연 writer는
이름이 바뀐 파일에 계속 쓴다")를 이미 받아들이고 있다 — 그래서 프루닝은
**새 레이스를 만드는 대신 이미 인정된 레이스를 재사용한다.** 최악의 경우
낙오 이벤트 한 줄이 살아남고, 다음 프루닝이 가져간다.

- 활성 로그는 **커트오프 이전 이벤트가 실제로 있을 때만** 회전한다. 할 일이
  없는 프루닝이 로그 레이아웃을 흔들면 안 된다.
- 부분 삭제는 같은 디렉터리의 temp + rename(원자적)이라, 중단돼도 잘린 로그가
  남지 않는다.
- **날짜를 못 읽는 줄은 지운다** — 최근이라고 증명할 수 없는 줄을 남기는 건
  프루닝이 아니다. 대신 `malformed` 개수를 따로 출력해 리포트가 로드하던
  수와 조용히 어긋나지 않게 한다.
- `--dry-run`은 활성 파일도 **회전 후 이름으로** 미리 보여준다. 한 파일에
  "회전 + 재작성" 두 줄이 뜨면 실제로 일어나는 일과 다르게 읽힌다.
...
- ~~로그 프루닝~~ — 해결됨. `graphin usage prune --before`(§5.1). 32MiB 회전은
  그대로 남고, 프루닝은 그 위에서 도는 별개 명령이다.
- ~~DB 스키마 질의(`db.` node_id) 채택률 분리 집계~~ — 해결됨(§4.3, 리포트의
  `## Target` 절). **다만 아직 값을 본 적은 없다** — 계측이 쌓인 유일한
  워크스페이스(kinder)에 `.graphindb.json` 스냅샷이 없어 db 분모가 0이고,
  리포트는 그럴 때 절을 통째로 감춘다(0%로 오독되면 안 되므로).
- `PostToolBatch` 활용(배치 경계 정밀화) — v1은 `parallel` 휴리스틱으로 충분.
  지금 데이터로는 이 휴리스틱이 틀렸다는 증거가 없다.
