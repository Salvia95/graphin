...
- **code vs db 타깃 분리**: 런이 참조한 노드 id의 네임스페이스(`db.` 접두사)로
  채택/폴백을 따로 낸다. 단위는 윈도우가 아니라 **런**이다 — "스키마 질문에
  graphin이 채택되는가"는 런의 질문이고, 한 윈도우가 code 런과 db 런을 하나씩
  가질 수 있다. code와 db는 **분할이 아니라 겹치는 두 모집단**이다(한 런이
  양쪽을 다루면 양쪽에 센다). search가 돌려준 `result_ids`도 분류에 쓴다 —
